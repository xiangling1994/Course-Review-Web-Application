...
def rating(request, pk, profid):
    ...
    if request.method == "POST":
        ...
        if form_helpfulness.is_valid() and form_clarity.is_valid() and form_easiness.is_valid() and form_textbook.is_valid():
            ...
            helpfulness_average_new = (float(rating_value_helpfulness) + float(helpfulness_db_val)) / 2.0
            clarity_average_new = (float(rating_value_clarity) + float(clarity_db_val)) / 2.0
            easiness_average_new = (float(rating_value_easiness) + float(easiness_db_val)) / 2.0
            textbook_average_new = (float(rating_value_textbook) + float(textbook_db_val)) / 2.0
            ...
            overall_rating_new = (float(overall_rating_db) + criteria_average) / 2.0
    ...
    return render(request, 'viewcourse/rating.html',{'criterias':criterias[0].course, 'form_helpfulness':form_helpfulness, 'form_clarity':form_clarity,  'form_textbook':form_textbook, 'form_easiness':form_easiness, 'current_professor':current_professor, 'username':username})
